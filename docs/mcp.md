...
Build `ob` from this repository and keep the resulting path stable:
...
```json
{
  "mcpServers": {
    "onebox": {
      "command": "/Users/you/src/onebox/bin/ob",
      "args": [
        "--config",
...
for it. `require_approval` is included in typed results so future execution and
control-plane adapters can enforce the same project policy; the current MCP has
no execution tool regardless of its value.

These values do not sandbox a shell-capable agent from the local CLI. Until the
planned capability broker and signed approvals exist, only grant MCP access—not
general shell access—when policy must be a hard boundary.
