...
`, application)
	...
	t.Cleanup(func() {
		...
		defer cancel()
		_ = exec.CommandContext(cleanupCtx, "docker", "network", "rm", network).Run()
	})
	...
	if out, err := exec.CommandContext(ctx, "docker", "network", "rm", network).CombinedOutput(); err != nil {
...
`, application, network)
	...
	if out, err := exec.CommandContext(ctx, "docker", down...).CombinedOutput(); err != nil {
		t.Fatalf("tear down release with proxy attached: %v\n%s", err, out)
	}
	if err := exec.CommandContext(ctx, "docker", "network", "inspect", network).Run(); err != nil {
		t.Fatalf("external application network was removed: %v", err)
	}
	proxy, err := exec.CommandContext(ctx, "docker", "ps", "-q", "--filter", "label=com.docker.compose.service=proxy", "--filter", "network="+network).Output()
