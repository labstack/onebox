...
func (e *Engine) gateSteps() []string {
	steps := append([]string{}, e.App.JobOrder()...)
	if h, ok := e.App.Hooks["migrate"]; ok && h.Run != "" {
		for _, s := range steps {
			if s == "migrate" {
				return steps
			}
...
func (e *Engine) onVerifyFailure(ctx context.Context, jw *journal.Writer, releaseID, prev string, verr error) error {
	...
	prevCompose := release.PathsFor(e.App.App).Releases + "/" + prev + "/compose.yaml"
...
func (e *Engine) jobDataEffect(job string) string {
	if w, ok := e.App.Workloads[job]; ok && w.IsJob() && w.DataEffect != "" {
		return w.DataEffect
	}
	return "unknown"
}
...
func (e *Engine) jobRollbackPolicySafe(service string) bool {
	...
	return effect == "none" || e.App.Deployment.MigrationPolicy == "expand-only" && effect == "migration"
}
