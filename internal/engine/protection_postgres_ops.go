...
// PruneServiceBackups expires everything outside the declared retention. It is
// separate from taking a backup because the order matters: expiring first would
// mean briefly holding one generation fewer than the policy promises.
func (e *Engine) PruneServiceBackups(ctx context.Context, service string) error {
	_, policy, err := e.protectedService(service)
	if err != nil {
		return err
	}
	generations := fmt.Sprint(policy.Retention.MinimumGenerations)
	st := e.ui.Step("prune "+service+" (retain "+generations+")", false)
	if _, err := e.runWalg(ctx, service, "delete", "retain", "FULL", generations, "--confirm"); err != nil {
		st(err)
		return err
	}
	st(nil)
