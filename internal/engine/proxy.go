...
func (e *Engine) EnsureProxy(ctx context.Context, deployID string, breakLock bool) (err error) {
	...
	hash, err := proxy.StageForAppManaged(localCfg, staging, e.Spec.Proxy.Image, discoveryImage,
		e.Spec.Name, e.Spec.Proxy.Network, e.Spec.Proxy.Entrypoints, e.Spec.HasExactTerminatingTLS(),
	...
	jw := &journal.Writer{T: e.T, Names: e.names(), Dir: e.names().HostDir() + "/journal", DeployID: deployID, Operator: journal.DefaultOperator(), GitSHA: e.Opts.GitSHA, ConfigHash: e.Opts.ConfigHash, Runner: &e.Opts.Runner}
