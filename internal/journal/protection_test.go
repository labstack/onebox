...
func TestProtectionStepIdentityIsDeterministicAndStructural(t *testing.T) {
	first, err := ProtectionStepID("backup_create", "database", "stream-artifact")
	if err != nil {
		t.Fatal(err)
	}
	second, err := ProtectionStepID("backup_create", "database", "stream-artifact")
	if err != nil {
		t.Fatal(err)
	}
	different, err := ProtectionStepID("restore_test", "database", "stream-artifact")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAppendProtectionReconcilesRetryAfterStreamDisconnect(t *testing.T) {
	stepID, err := ProtectionStepID("backup_create", "database", "stream-artifact")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := writer.AppendProtection(context.Background(), candidate); err != nil {
		t.Fatalf("append protection after output loss: %v", err)
	}
	if readCount != 2 {
...
func TestLookupProtectionTerminalResultAfterClientOutputLoss(t *testing.T) {
	stepID, err := ProtectionStepID("backup_create", "database", "record-result")
	if err != nil {
		t.Fatal(err)
	}
	record := Record{
		DeployID: "backup-2", Phase: "backup", Event: "finish", Status: "ok", TS: "2026-08-07T12:01:00Z",
		OperationKind: "backup_create", Service: "database", ProtectionStepID: stepID, ProtectionAttempt: 1,
		TerminalResult: &ProtectionTerminalResult{State: "succeeded", EvidenceID: "backup-generation-9"},
	}
	encoded, err := json.Marshal(record)
	if err != nil {
		t.Fatal(err)
	...
	result, ok, err := LookupProtectionTerminalResult(
...
func TestProtectionJournalRejectsUnpinnedHelperAndInvalidIncompleteResource(t *testing.T) {
	stepID, err := ProtectionStepID("backup_create", "database", "stream-artifact")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := validateProtectionRecord(record); err == nil {
		t.Fatal("invalid protection journal metadata was accepted")
	}
}
