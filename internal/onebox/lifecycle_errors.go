...
var lifecycleFailureDefinitions = map[string]lifecycleFailureDefinition{
	"assurance_stale":                              {"continuous assurance evidence is no longer current", "ob doctor --output json"},
	...
	"backup_stale":                                 {"the latest recoverable point is older than policy permits", "ob doctor --output json"},
	"backup_target_not_independent":                {"the backup target shares the protected failure domain", "ob validate --output json"},
	"backup_target_unauthorized":                   {"the backup target credentials are unavailable, unsafe, or unauthorized", "ob validate --output json"},
	"backup_target_unknown":                        {"the protection policy selects no declared backup target", "ob validate --output json"},
	"backup_target_unreachable":                    {"the selected backup target cannot be reached", "ob validate --output json"},
	"disk_pressure_critical":                       {"a relevant filesystem lacks safe headroom for a space-increasing mutation", "ob doctor --output json"},
	"drill_deferred_capacity":                      {"a restore drill was deferred before materialization because aggregate staging headroom is insufficient", "ob doctor --output json"},
	...
	"replay_continuity_broken":                     {"the native replay sequence has a gap inside the required recovery window", "ob doctor --output json"},
	"restore_drill_schedule_too_sparse":            {"the restore-drill cadence cannot keep restore proof current", "ob validate --output json"},
	"restore_state_stale":                          {"live service, volume, or repository state changed after restore planning", "ob doctor --output json"},
