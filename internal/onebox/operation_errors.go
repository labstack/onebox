...
var operationFailureDefinitions = map[string]OperationFailure{
	...
	"job_history_failed": {
		Message: "the job's retained execution records could not be read",
		Command: "ob status --output json",
	},
	"job_input_invalid": {
	...
	"job_logs_failed": {
		Message: "the host-supervised job journal could not be read",
		Command: "ob job history <job> --output json",
	},
	"logs_failed": {
