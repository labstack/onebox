...
func TestDefaultProxyRenderingRemainsStable(t *testing.T) {
	const legacyCompose = `# generated by onebox — do not edit (ob proxy apply owns this file)
services:
  proxy:
    container_name: onebox-proxy
    image: traefik:v3.7
...
`
	...
	if got := string(RenderCompose("", "", false, nil)); got != legacyCompose {
		t.Fatalf("projects without additional entrypoints must keep the legacy Compose output:\n%s", got)
	}
	if got := string(renderStaticConfig(nil)); got != DefaultStaticConfig {
		t.Fatalf("projects without additional entrypoints must keep the legacy static config:\n%s", got)
	}
}
