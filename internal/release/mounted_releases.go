...
//
// The mounts are the evidence, not the onebox.release label. A retained container
...
func MountedReleases(ctx context.Context, target transport.Transport, names app.Names) ([]string, error) {
	command := "docker ps -a --no-trunc --filter label=onebox.app=" + q(names.App) + " --format '{{.Mounts}}'"
