...
- [x] 2.1 Declare the workload structs per role, the service, environment, proxy, registry, notification, secret, verification, and runtime shapes, mirroring the existing model.
- [x] 2.2 Decode the expanded document with `KnownFields(true)`, selecting a workload's struct by its declared role.
...
- [x] 2.4 Keep `expand()` unchanged and assert every shorthand form still normalises identically.

## 3. Move constraints into the validation table

- [x] 3.1 Move every enum, pattern, and bound into one table keyed by field path, each entry carrying the message its violation produces.
- [x] 3.2 Port the hostile-value tests unchanged: timezone, image reference, registry server and username, absolute path, repository path, URL path.
- [x] 3.3 Assert the ordinary values real projects write still load, including an IANA zone, a digest-pinned image, a registry with a port, and a nested env-file path.
- [x] 3.4 Preserve the near-miss suggestion and assert it for a typo in each role.
...
- [x] 5.1 Delete `internal/app/schema.cue`, the rewording layer, the path extractor, the per-role pre-validation pass, and the `cuelang.org/go` dependency.
- [x] 5.2 Run the equivalence harness and require an identical result for every case, every project, and every digest.
- [x] 5.3 Confirm the loader's 23 typed error codes are unchanged in identity and in the command each names as the resolution.
