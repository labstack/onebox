...
- [ ] 1.3 Update status and observation paths that match containers by name in `internal/engine/status.go` and `internal/engine/status_snapshot.go`.
...
- [ ] 2.2 Detect a foreign container holding a derived name by label before any container is started, stopped, or renamed, failing with the holder named.
