...
Backup applies the principle to data recovery. A service reports `Managed` only
while, **all at once**:
...
Knowing that `retainReleases: 5` was Onebox's choice rather than yours is the
