...
- **Derived, stable names.** Workload containers use the uniform
  `<app>-<component>-<replica>` grammar, such as `shop-web-1`; managed services
  are `onebox-<service>`, such as `onebox-postgres`; persistent and
  provider resources include `shop_default`, `onebox_postgres_data`, `onebox_services`,
  and `onebox-ingress`. The application and service networks are declared external
...
owns — `extends`, a fixed `container_name`, `network_mode`, labels in the `onebox.`
