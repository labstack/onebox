...
target. Everything below is achieved with short-lived processes, durable state,
...
**Digest-bound approvals.** A grant covers one exact plan, target, input set,
risk classification, operator and expiry. A changed or expired plan needs a new
grant.

**Drift detection.** A target-side artifact differing from what the plan bound is
...
target and whether it changes anything; `ob exec` is the one row without a
