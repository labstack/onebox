...
- **The host layout.** Releases, the journal, and supporting services under
  `/var/lib/onebox/app` remain until you remove them.
...
`ob destroy` keeps volumes unless you explicitly ask otherwise. When an active
release exists, Onebox tears it down with that release's recorded project
snapshot and interpolation environment. A newer project file can therefore
migrate a Compose-defined database to a managed service without making the old
release impossible to parse during teardown.
