...
two generations while a rolling replacement runs, which is the same thing a
deploy does for any other environment change. A failure recovers every workload to the old
