...
Only the newest deploy is recoverable. Once a later deploy settles the host's
release state — completing, aborting or rolling back — the older interrupted one
is history rather than pending work, and resuming it would re-activate a release
the host has already moved past. `ob status` stops reporting it; `ob audit` still
shows the run.
