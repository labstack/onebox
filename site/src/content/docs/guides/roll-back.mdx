...
Start by asking the target, not your memory:
...
`deployment.retain_releases` (default `5`) decides how many completed release
directories stay available. Rollback re-points `current` at one of them.
...
A force flag cannot turn an unsupported operation into a safe one.
:::
...
Both read the append-only journal and the durable phase markers written during
the deploy. They choose only recovery paths the journal can prove are safe; when
safety cannot be proven, they stop and expose honest recovery choices rather than
guessing.
...
ob logs           # the current log stream
ob status --output json
```
