...
`ob resume` finalizes only when the recorded activation, the current release,
the serving manifest and its recorded predecessor, and the live workload labels
all agree. One disagreement returns `finalize_refused` and changes nothing.

Only the newest deploy is recoverable. Once a later deploy reaches a terminal
state it has replaced every workload and activated its own release, so the older
interrupted one is history rather than pending work — resuming it would
re-activate a superseded release. `ob status` stops reporting it; `ob audit`
still shows it as incomplete.
