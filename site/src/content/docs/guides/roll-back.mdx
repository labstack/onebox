...
`deployment.retainReleases` (default `5`) bounds ordinary history while always
...
its `onebox.release` label names an older release; a missing, unhealthy, incomplete
...
- **A job with `dataEffect: Migration` or `destructive` constrains the gate.**
