...
summary: How to declare a migration job, what dataEffect controls, how job result evidence is captured and verified, and how the migration-backup policy gate works.
sidebar:
  order: 3
read_when:
...
`dataEffect` is **required** on a job, and it is what the rollback and abort
gates read:
...
`deploymentPhase` decides whether it runs during deployment: `none` (default),
`pre_release`, or `post_release`. `operatorRun` independently controls explicit
invocation (`allowed` or `disabled`); release-phase jobs default to `disabled`.

## Report what actually happened

A job can write JSON or `key=value` data to `$ONEBOX_RESULT_FILE` using the
...
Migrations and unknown data effects use the **strong ceremony**, where the
operator types back what the plan acts on: the release ID for a deploy, the job
name for a job run. That is the difference between confirming a routine deploy
and confirming one that may not be reversible. A migration job asks whether or
not approval policy is enabled.

## Release-wide policy

```yaml
deployment:
  migrationPolicy: Manual   # manual · auto · expand-only
```
...
**open** after a `dataEffect: Migration` job runs, so a failed verify can roll
