...
description: Migration jobs, data effects, result evidence, and the backup-evidence gate.
...
It is the only artifact here you author yourself — the plan, the grant and the
receipt are all produced by `ob` — so start from a skeleton with your plan's
resources already filled in:

```sh
ob backup-evidence template --plan ob-plan.json > backup-facts.json
```

It uses `onebox.run/migration-backup-facts/v1alpha1` and records artifact,
integrity, restore-test and key-usability facts — never backup bytes or secrets.
...
| `resources[].restore_test` | `state` is `passed` or `not_tested`; `method`, `tested_at`, `validation_digest` optional |
