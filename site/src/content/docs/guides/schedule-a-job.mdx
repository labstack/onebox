...
description: Bounded host timers, deploy serialization, failure status, and the cron forms Onebox refuses.
summary: How to declare a scheduled job, bound its run time, choose catch-up behavior, inspect failures, and understand which cron expressions are refused at load.
sidebar:
  order: 4
read_when:
