...
read_when:
  ...
  - "Handing a large result set from one step to the next"
...
### A result set is a reference, not an output

Those bounds are the contract, not a current implementation limit. An output
names *what* a later step should work on; it is not the transport for the work
itself. A step that produced one identifier, a cursor, a batch number, or a
content digest fits comfortably. A step that produced ten thousand row IDs does
not, and raising the numbers would not help: outputs reach the next step as
environment variables, and the checkpoint they live in is what makes a resume
cheap to read and safe to replace whole.

So when a step's real result is a set, the application persists the set in its
own storage and hands over one reference to it:

```yaml
      steps:
        - id: sync
          command: ["./ingest", "sync"]
          # Not the IDs themselves: `sync` writes the batch to its own database
          # and returns the key that names it.
          outputs: [BATCH]
        - id: index
...
`index` reads the batch back by its key, so the set can be any size and a resume
still skips a completed `sync` and works on exactly what it produced.

The alternative is worth naming, because for many jobs it is the better one: do
the whole thing in one step. If the second stage is cheap to repeat, or a
periodic reconciler already repairs whatever it missed, one command that does
both needs no batch table, no reference, and no step boundary to get wrong.
Declare steps when you want them recovered independently — not to pass data
between two halves of one operation.
