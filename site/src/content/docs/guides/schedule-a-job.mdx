...
version 252. Every scheduled job therefore needs systemd 252 or newer on the
host, not only one with inputs: the record's `trigger` is part of the contract,
and on an older systemd a timer firing would both be recorded as manual and
consume an operator's pending inputs. `ob preflight` and `ob deploy` refuse an
older host before anything is staged. Ubuntu 24.04 and Debian 12 qualify.
