...
ONEBOX_UNIT=onebox-job-<job> -o cat` on the host is the raw history:
...
`dataEffect` is required on every job, scheduled or not. A nightly report is
...
The three properties are independent. `deploymentPhase` controls deploy hooks,
