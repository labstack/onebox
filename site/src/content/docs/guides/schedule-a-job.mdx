...
An exclusive host-fired job takes an application-wide kernel lock for its whole
...
When the manual job also declares `schedule`, Onebox submits this run to the
...
with `ob schedule history <job>` and `ob schedule logs <job>`.
