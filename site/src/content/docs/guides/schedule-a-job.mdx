...
Only a job with `data_effect: none` accepts inputs. Every operator invocation
...
a native Onebox job with `schedule`, `deployment_phase: none` (the default), and
...
When the operator job also declares `schedule`, Onebox submits this run to the
...
`ob job plan` accepts a job when `operator_run` is `allowed`. This property is
independent of deployment: `deployment_phase` decides whether the job runs in
the deploy graph, while `operator_run` decides whether an operator may invoke
it. By default, phase `none` allows operator runs; release-phase jobs disable
them unless explicitly enabled.

A declared `deployment_phase: none` job remains in the digest-pinned release runtime but
...
operator_run: allowed            # optional: also permit ob job plan/run
```

The three properties are independent. `deployment_phase` controls deploy hooks,
`schedule` controls timer activation, and `operator_run` controls explicit
invocation. A job declaring `schedule:` and `deployment_phase: pre_release`
runs both at its cron time and on every deploy. For a timer-only job, leave
`deployment_phase` at `none`; set `operator_run: disabled` too if operators
