...
names may not be `proxy`, `discovery` or `services`. Custom Traefik objects may
...
2. Stop and remove the old scheduled job and backup units, so nothing runs
   ...
   APP=shop
   ENV=production
   systemctl list-unit-files --no-legend "ob-$APP-*" "ob-backup-$APP-*" | awk '{print $1}' \
     | xargs -r systemctl disable --now
   rm -f /etc/systemd/system/ob-$APP-* /etc/systemd/system/ob-backup-$APP-*
...
4. Copy every volume into its new name, with the new labels:
   ...
   for old in $(docker volume ls -q --filter label=ob.app=$APP | grep "^ob_${APP}_"); do
     ...
     labels="--label onebox.app=$APP"
     [ -n "$service" ] && labels="$labels --label onebox.service=$service"
...
5. Move the state and rewrite the owner record:
   ...
   docker network rm ${APP}_default ob_$APP ob-ingress
   ```
