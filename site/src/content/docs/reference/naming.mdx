...
| Plural | Singular |
| --- | --- |
| `workloads` `services` `registries` `environments` `hooks` `notifications` `backup_targets` `external_services` | `runtime` `deployment` `proxy` |
...
| **target** | a backup destination — `backup_targets`, `backup.target` | the machine you deploy to |
