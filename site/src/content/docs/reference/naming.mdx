...
`--force` means break a held lock, printing the holder first. A flag that
proceeds past a refusal which could lose data says so in its own name —
`--break-migration-gate`, `--allow-destructive-mounts`.

One exception, and it should not survive: on `ob proxy apply` and `ob bootstrap`,
`--force` *also* overrides a cross-app proxy config conflict, which is not a lock
at all. That second meaning needs its own flag.
