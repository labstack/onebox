...
(or `ob-`), and `proxy` and `discovery` are not valid service names.
...
**Every other resource Onebox derives is `onebox_<app>_…`.** Volumes, service
Compose projects, the service network and restore-drill resources:
`onebox_shop_postgres_data`, `onebox_shop_postgres`, `onebox_shop`. They carry
the application, even for a managed service, because they hold that
application's data. They join with underscores because an application or
component name cannot contain one, so each name maps back to exactly one
application, component and volume — a hyphen, which names may contain, could
not promise that. The one exception is the application's own Compose network,
`shop_default`, which is the name Compose gives it.
...
| `onebox_<app>_` | underscore | the application's volumes, service projects and networks |
