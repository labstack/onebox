...
runs one application, and the `ob.app` label records which. So `docker ps`
...
(or `ob-`), and `proxy`, `discovery` and `services` are not valid service
names.
...
`ob.app` label records which, and `ob destroy` releases the host only after
