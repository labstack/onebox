...
A manual job carrying one of those effects asks whether or not approval policy
...
| --- | --- | --- | --- |
| Finite envelope | yes | no | `ob approve` · `ob audit` · `ob backup status` · `ob canonical` · `ob doctor` · `ob eject` · `ob execution list` · `ob execution inspect` · `ob init` · `ob job plan` · `ob plan` · `ob preflight` · `ob preview` · `ob schedule history` · `ob schedule list` · `ob schema` · `ob secrets list` · `ob status` · `ob validate` · `ob version` |
...
| Operator passthrough | finite only | yes | `ob logs` · `ob schedule logs` |
