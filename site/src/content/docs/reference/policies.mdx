...
| `migration_backup_max_age` | — | Maximum age of accepted evidence |
...
`ob abort` · `ob backup-evidence create` · `ob bootstrap` · `ob canonical` ·
`ob deploy` · `ob doctor` · `ob eject` · `ob plan` · `ob preview` ·
`ob proxy apply` · `ob resume` · `ob rollback` · `ob service apply` ·
`ob status` · `ob validate`

Anything not listed is human output only. `ob doctor` and `ob version` also
accept `--json` for their reports.
