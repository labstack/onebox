...
`cmd/ob/docs_test.go` asserts that every command with structured output, and
every schema identity the binary emits, appears on this page. A new one that is
not documented here fails the build.
:::
