...
configuration. Declare `proxy.config` with dynamic YAML or TOML files to extend
...
In both modes, the filenames `onebox.yml` and `onebox.yaml`, the `dynamic/`
mountpoint, and HTTP or TCP router or service names beginning with the derived
`<app>_` prefix are reserved for generated discovery output. `ob deploy`,
`ob bootstrap`, and `ob proxy apply` refuse an incompatible configuration with
migration guidance rather than silently starting a proxy with missing or stale
routes.
