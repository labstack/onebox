...
:::caution[One breaking change already happened]
`onebox.run/v1` was redefined in place rather than superseded by a v2. A project
written against the earlier contract — which classified a Compose file *you*
wrote — does not load, and must be authored fresh. Compose used to be an input
Onebox read; it is now an artifact Onebox generates.
:::
