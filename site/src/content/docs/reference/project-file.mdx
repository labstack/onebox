...
A project that hands Onebox a Compose file *you* wrote does not load. Compose is
an artifact Onebox generates from the declaration, so the runtime is derived
rather than classified. See [Why Compose is generated](/explanation/generated-compose).
:::
