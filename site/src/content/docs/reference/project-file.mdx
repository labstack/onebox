...
but not `TRAEFIK_PROVIDERS_*` or `TRAEFIK_CONFIGFILE` static overrides.
...
bare catch-all. Exact and wildcard claims that overlap on the same entrypoint,
...
`certificatesResolvers.letsencrypt` there and omit `proxy.dns_challenge`.
