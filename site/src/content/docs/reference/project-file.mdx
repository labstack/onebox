...
## Migrating route declarations

Every ingress rule is explicit and uses `hostname` for both exact hosts and
wildcard hosts. The contract identity remains `onebox.run/v1`; update retired
route fields as follows:

```yaml
# retired scalar route shorthand
...
```yaml
# retired wildcard route
routes:
  - wildcard_suffix: preview.example.com
    port: 3000

# current: a complete leftmost wildcard label, matching one label only
routes:
...
`routes`. Ingress is always declared through `routes`; the scalar `domain`/`port`
route shorthand is not accepted.
...
release: the next deploy ships a new one, and `deployment.retain_releases`
