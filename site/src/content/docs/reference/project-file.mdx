...
`onebox-postgres` for a managed service and `onebox-proxy` for the proxy. Persistent and provider names include `ob_shop_postgres`,
`ob_shop_postgres_data`, `shop_default` (the external application network),
`ob_shop` (the external service network), and `onebox-ingress`. These are contract:
