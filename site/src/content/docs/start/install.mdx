...
:::note[No release is published yet]
The pipeline below is configured and verified in CI, and the first
`vYYYY.M.REVISION` tag publishes it. Until that tag exists, these downloads —
and the Homebrew and Scoop commands, which fetch the same assets — return 404.
