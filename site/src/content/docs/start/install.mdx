...
the version it observed alongside the result. Distribution packages are a common
source of a partial install: Ubuntu's `docker.io` gives a working daemon and no
Buildx at all, and its `docker-buildx` 0.30.1 accepts `imagetools inspect
--format`, ignores it, and exits 0, which is why the check probes the capability
