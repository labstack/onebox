...
2. **Confirm which runner will execute plans.**
   ...
   Releases use `vYY.M.SEQUENCE`, for example `v26.8.1` for the first release
   in August 2026. Months are unpadded and the sequence increases for each
   release in a UTC calendar month. Checkout builds use Git-derived provenance
   and stay visibly distinct from a release.
...
value, while the download URL uses the full `vYY.M.SEQUENCE` tag.
...
sudo apt install ./onebox_26.8.1_linux_amd64.deb

# Fedora or RHEL (choose amd64 or arm64)
sudo dnf install ./onebox_26.8.1_linux_amd64.rpm
```
...
The application tag `v26.8.1` is intentionally CalVer-shaped, but the Go module
path does not declare a `/v26` module major. Use a precompiled release asset or
build from source; `go install github.com/labstack/onebox/cmd/ob@v26.8.1` is not
