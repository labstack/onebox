...
## Distribution, pending the first tag

Configured and verified in CI on every pull request, and published by the first
`vYYYY.M.REVISION` tag. No release exists yet, so the install commands in the
documentation begin working at that tag and not before.
